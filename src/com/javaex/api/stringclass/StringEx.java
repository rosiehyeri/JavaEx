...
//		stringBasic();
	...
	private static void stringBasic() {
		...
		System.out.println(s1 == s2); // 주소 비교 : 리터럴 vs 뉴 객체 생성
		System.out.println(s1 == s3); // 주소 비교 : 리터럴 vs 리터럴
	...
	private static void usefulMethods() {
		...
		System.out.println("toLowerCase: " + str.toLowerCase()); // 전부 대문자로
		...
		index += "Programing".length(); // 검색 시작 위치를 뒤로 이동
		index = str.indexOf("Programming", index); // 새로운 시작 위치에서 검색
		System.out.println("3rd search: " + index); // 검색할 것이 없으면 -1 나옴
		...
		// 문자열 데이터는 불변이다(immutable)이다
		// 여러가지 메서드를 통해 변하가 발생 했더라도 원본은 유지된다
